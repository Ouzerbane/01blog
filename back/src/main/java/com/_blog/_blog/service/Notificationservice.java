...
public class Notificationservice {
    ...
    public NotificationDto notificationsRead(UUID Id ,AuthEntity auth) {
        NotificationEntity noti = notificationRepo.findByIdAndUserId(Id, auth.getId());
        noti.setRead(!noti.isRead());
        noti = notificationRepo.save(noti);
        return NotificationDto.builder().id(noti.getId()).message(noti.getMessage()).status(noti.isRead()).build();
    ...
    public List<NotificationDto> getNotificationsAndMarkRead(UUID userId) {
        ...
        return notifications.stream()
                .map(f -> new NotificationDto(f.getId(), f.getMessage(), f.isRead()))
